...
class FxTotal extends React.Component {
    constructor(props) {
        super(props);
        this.finalExamTotal = this.finalExamTotal.bind(this);
    }

    finalExamTotal(level) {
        ...
        for(let j = 0 ; j < currentLevel.length ; j++) {
            ...
            if(level === 'ES011' || level === 'ES012' || level === 'ES020' || level === 'ES030' || level === 'ES040') {
                ...
                total = currentLevel[j]['rlgvosswe'];

            } else {

                total = currentLevel[j]['rlgvos'];
    ...
    render() {
        return (
            <div>
                <h2>Final Exam Total</h2>
                <AreaChart width={1000} height={400} data={this.finalExamTotal(this.props.level)}>
